...
**Screens are the one layer GameTest and JUnit are blind to.** Geometry is asserted server-side and the layout algebra has unit tests, but a gauge filled from the wrong end passes both. `python tools/shoot_screens.py` opens all six in a running `runClient` and screenshots them - that is the acceptance evidence, and it needs the game window foregrounded because devbridge has no world-interaction verb. **The guidebook sits in the same blind spot and is not covered by that script**, which is how #241 went unnoticed: every paragraph break in all 71 of the book's text pages is swallowed, so paragraphs run together with no space between them, and it had been shipping for releases. `GuidebookTests` proves a lang key exists and an icon resolves and passes throughout. Opening a page means a synthesized right-click with the book **in the selected hotbar slot** - read the slot from the player rather than assuming 0, because holding nothing reports exactly the same "no screen opened" as a page that failed to draw.
...
**Four machines, four verbs, and the split is the design** (#187, #188, #189 shipped 2026-08-16/17,
#236 on 2026-08-19). The test is what an operation does to the material:
