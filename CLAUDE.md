...
**One rule is blockstate, not block, so no tag can express it:** *wet farmland holds, dry farmland is taken.* Irrigation defends a plot and an abandoned one dries out and goes back to the dump, which makes the P1.10 water economy a reclamation defence rather than only an input. Keyed on `BlockStateProperties.MOISTURE` rather than on `minecraft:farmland`, so modded farmland is covered without being named. It is also the one place encroachment destroys player investment (a crop on dry farmland goes with it), deliberately gated behind "you let it dry out". Tuning is those two plus `hostile_ground`, `frontier_anchor`, `frontier_cover` and the biome tag `encroaches`, over the `reclamation` config block - the mechanic is inert outside the garbage biomes. `encroachOnce` is the static test entry point; the sweep owns targeting (config gate, biome, heightmap), which is why the GameTests can run it on a plain plot.
