...
**The decant pond survives regrowth because the MEMORY never claims its cell**, not because anything checks for water. `TailingsHeapFeature` records the count of tailings rather than the column height, and a pond column is cut one block below the plateau - so the highest cell regrowth can target is the top tailings block and the water above it is out of reach. Water is a `canBeReplaced` block, so the obvious version would have filled the basin in one block at a time, silently, over a long time, in a region nobody watches.
...
- **Teardown has a JEI category and has had one since Phase 3 shipped.** `registerCategories` builds a `SalvageCategory` for `TEARDOWN` sized off `TeardownData.all()`, `registerRecipes` feeds it, and the Recompile Workbench is its catalyst. *(This line read "Teardown JEI is deferred to Phase 3 (only an example recipe exists; its locked-recipe overlay is the Phase 3 risk spike)" long after both halves stopped being true - there are **twelve** files of `"type": "recompile:teardown"`, not one example - and it was the most misleading sentence in this file, because it tells a reader a shipped viewer category does not exist and so invites them to build it a second time.)* EMI is not wired.
...
**What a pack can change without a mod release is written down**: `docs/pack_extension.md` - the three sewer loot tables and every pull stream, the ten public recipe types, the tag surface, and how to add guidebook entries from another namespace. It also states what a pack CANNOT reach from data, which is the half people discover the hard way: the viewers read the mod's bundled JSON rather than the live registry, so a retuned pull stream is right in-world and stale in JEI.
