...
**Teardown is the namesake but no longer the spine** (owner, 2026-09-06, locked as P3.10 in `../trashlands/docs/design_decisions.md`). The pack's progression is a **freight ladder**: each tier opens by shipping named processed goods to the market's off-site buyer, and delivery quotas rather than a scrip balance are the gate, because a currency gate is fungible and collapses every region into one payout number. Teardown moves **off the critical path to an optional discovery track** - Satisfactory's split between Space Elevator phases (mandatory, delivery-gated) and the MAM (optional, discovery-driven). It is demoted, not deleted, and `docs/structural_analogs.md` carries the survey the ruling was made against. **Standalone, the mod still ships teardown exactly as before**; what changed is what the pack builds its ladder out of, and the engine half of the freight system is not built yet.
