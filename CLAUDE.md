...
**What this is:** a standalone **NeoForge** mod (MC 26.1.2). It ships the garbage-world systems (worldgen, Blocks of Garbage, sorting, mound regrowth, the reclamation ladder, the market) that the **Trashlands** modpack is built on, plus **teardown-as-knowledge** - disassemble items to recover their recipes, not just their materials. Mod id / package: `recompile` / `com.flatts.recompile`.

**Teardown is the namesake but no longer the spine, and it no longer teaches** (owner, 2026-09-06, locked as P3.10 in `../trashlands/docs/design_decisions.md`). Four systems, four jobs, no overlap: the **market** is the only source of knowledge (every Blueprint bought with scrip), **freight quotas** are tier progression (delivery of named processed goods, not a scrip balance - a currency gate is fungible and collapses every region into one payout number), **teardown** yields **function** (working components you cannot forge from any amount of scrap), and **sorting** yields bulk materials.
...
the `fragment_assembly` type: wrong, because the **Sequencer** creates those fragments and a spawn-egg
Blueprint is four of them, so deleting the item would have broken the exception the same audit had just
sanctioned. What actually died is teardown GRANTING them, and the item is **renamed to Spawn Egg
Fragment** (owner) now that amber is its only source and a creature Blueprint its only destination. A
registry id is save data, so `RCRegistryAliases` keeps `recompile:idea_fragment` resolving - and the
mechanism in 26.1 is `IRegistryExtension.addAlias`, **not** `MissingMappingsEvent`, which does not exist
on this version and fails to compile with a bare `cannot find symbol`.

**A signature component is what an object IS, and each one belongs to a single object.** Washing machine
to Motor, Broken Hydroponics Bay to Pump, dead Hauler to Solar Panel, Broken Spawner to Machine Frame.
The fridge is the deliberate exception - a three-way lottery of Motor, Pump or Bulb, by the owner's
2026-08-12 ruling, which survives the move to function. Three of these first shipped with a signature
stacked ON TOP of the component pool that was already there, handing back two components where main gave
one; `no_two_objects_yield_the_same_signature_component` missed it because it read `results()` alone, and
a guaranteed component can equally be a pool with no filler entry. It reads **guaranteed output** now.

**Status:** Phases 0 through 2.17 are shipped (through the full reclamation ladder - Grass, Vegetation, Farming, Trees, Animals), **Phase 3 (teardown-as-knowledge) shipped 2026-08-02**, **Phase 4's region system and its first frontier region (the demolition yard) are shipped**, **Phase 5 (mound regrowth) shipped 2026-08-05**, and **Phase 7's themed Nether was pulled forward and shipped 2026-08-19** (the compacted depths, plus the slag chain that gets you there: the Cupola's byproduct, the Slag Furnace, and the only obsidian in the game). **v0.20.0 is released** (2026-09-07, the **freight ladder** - a Freight Terminal, eight delivery phases, and the tier they set is what the Buy Terminal will sell against - plus the spine change beneath it: **teardown no longer teaches**, every Blueprint is bought, and what teardown yields instead is FUNCTION, the signature component an object IS. The **Motor** is salvage or nothing, and a **terminal is repaired from a Broken Terminal** rather than built; v0.19.0 2026-09-05, the **Scrap Hauler** - a robot that works a square of chunks around its Depot and brings whole blocks back to it - the **market**'s two terminals and its company scrip, and **Sledgehammer combat**: sword-equivalent damage, knockback, shield-disabling, and enchantments it never took before; v0.18.0 2026-09-04 the **Municipal Aquarium** - the only prismarine, coral, sponge, sea lantern and guardian in the game, plus moss and pale moss - and **tire dumps** across the household sprawl, the only rubber and the only fire that never goes out; a **Dried Bouquet** rehydrated on a water cauldron for the four tall flowers and the large fern, six more plants placed as exhibit stock and sewer dressing, and hopper/pipe automation for the Tree Nursery; v0.17.0 2026-09-03 the Garbage Vacuum and its screenless Charging Station, plus the battery chain that gates both - a Depleted Battery found in household waste is torn open for scrap and for the blueprint that makes a live one; v0.16.0 2026-08-31 two landmark structures - a decrepit cooling tower in the radioactive dump and brick smokestacks across the demolition yard, both with a vanilla spawner at the foot - plus cardboard, the first building family reachable with no tool, station or recipe; v0.15.0 2026-08-30 spawn eggs via amber and the Sequencer, the radioactive dump as the second frontier region, and AE2 made playable; v0.14.0 2026-08-21 Ancient Sculk plus the two cross-mod stopgaps and the recipe-collision fix; v0.13.0 2026-08-20 the Sintering Kiln and the last of the resource gaps - brewing, netherite and emeralds; v0.12.0 2026-08-19 the compacted depths, v0.11.0 on 2026-08-18 the sewers, v0.10.0 on 2026-08-17, v0.9.0 on 2026-08-12, v0.8.0 on 2026-08-11, v0.7.0 on 2026-08-05, v0.6.0 on 2026-08-04, v0.5.0 on 2026-08-02, v0.4.0 on 2026-08-01, v0.3.0 on 2026-07-30, v0.1.0 and v0.2.0 on 2026-07-27). The grey-to-living arc the ModJam entry is built around now plays end to end, and mounds are renewable quarries rather than a finite stock. `docs/roadmap.md` is the engineering build order and tracks per-phase status; **Phase 6 (the full loop) is what remains**. Phase 3's long-open **knowledge-vs-function question was decided on 2026-08-01: knowledge**, as Immersive-Engineering-style **Blueprint items** (spec `docs/blueprints_spec.md`, issue #95). Tear something down at the Workbench and you may come away with an **Idea Fragment**; enough fragments about one thing craft into a **Blueprint**; a **Filing Cabinet** found in Bulky Waste files them and joins the Scrap Network by placement; and the **Scrap Crafting Table** will run a `recompile:blueprint_crafting` recipe only while the sheet is in the player's inventory or in a cabinet in the same cluster. A vanilla crafting table needs no code to be excluded - blueprint recipes are not of type `minecraft:crafting`, so it cannot see them at all.
...
`unitTest` is enabled in `build.gradle` (moddev's JUnit integration, which runs `src/test/java` against a loaded mod context) and **`./gradlew test` runs 147 tests across 35 classes** (measured 2026-09-08 off `build/test-results`; it said 132 across 30 until then). `build` depends on `test`, so CI gates them. *(This line previously said no JUnit tests existed. That was wrong from PR #22 onward and went unnoticed until someone counted - a doc claiming a layer is empty is how it stays empty.)* **Use a unit test when the logic is pure** - `GeneratorState` (which reason a generator is idle), `ScrapBinContent` (item to bin appearance), the crumble curve's expected yield. No world, no rendering, no server means a GameTest is the wrong instrument and a slower one. GameTests remain where in-world behaviour is proven.
...
component via NeoForge's stack-backed `ItemAccessEnergyHandler`, registered per tier in
`RCBlockEntities`, and the **Charging Station** (no screen - the pedestal's set-down/pick-up, energy
in only, not in the Scrap Network because that tag routes items) charges it through that door. The
animation is Slime Rancher's vacpack in this game's idiom and costs no rule: a taken block is a
`VacuumedBlockEntity` (an ENTITY, so its renderer is ordinary like the Roach's - Mining Gadgets was
the reference and its transient-block-entity BER was deliberately not copied, because the pedestal
is this mod's one BER), the held model switches on `using_item` to a body plus an animated
`vacuum_intake` overlay from a texgen procedural style, and block dust streams into the nozzle. And
...
**The Scrap Hauler is this mod's quarry, and it is a Mob with the biology switched off** (owner,
...
of being a `LivingEntity` that must not act like one**: `LeachateBlock.sicken` carries an explicit
`instanceof` exemption, because sickening is a mob EFFECT and effects ignore `isInvulnerable`
entirely; `MobCategory.MISC`, so a machine eats no spawn cap; `setPersistenceRequired`, because once
nothing can destroy it, despawning is the only loss path left; and `tickDeath` removes it on the spot
instead of playing the death animation, so a `/kill` (the one thing `#minecraft:bypasses_invulnerability`
lets through) unlocks the Depot on the same tick. **The conservation invariant is the design: it
exists exactly once, as the item in the Depot's slot XOR the entity**, and six code paths can break
that - the GUI slot, the shift-click, the hopper face, breaking the Depot while deployed, a duplicated
Deploy, and the entity vanishing by any route - each with its own test in `ScrapHaulerTests`, the
...
calls or a hundred ticks without progress, then a 400-tick blacklist. The failure mode of an
indestructible machine is being STUCK, not dying, which is why Recall teleports rather than paths.
...
(cannot be bucketed dry; heaps regrow since P1.6-R, so it is the renewable half of that pair rather
than the merely large one). Four things about how it is built are worth
...
**What the number actually is, because this paragraph had it wrong by a factor of fifty.** It said weight 12 in `household_pulls`, about 1 in 52 pulls, "instead of alongside the conveniences like the bowl at 5". The bucket is weight **4** in the durable-goods pool - **1 in 2,500 pulls** - which makes it about nineteen times RARER than the bowl rather than commoner. Nothing regressed: the 2026-08-11 playtest found nine buckets in a fifteen-minute session, and the durable-goods pool was added to fix that class of problem - its own reason for existing is recorded about the shears, where weight 1 was the floor and still gave about one a mound. `FindRateTest` pins the result at roughly one every half hour. The prose simply never followed the fix. Read the rate off `FindRateTest`, which measures it, not off a sentence.
...
`household_sprawl` has an **empty `monster` list by design** - nothing hostile spawns in the starting biome, which is why food comes from tin cans and foraged mushrooms rather than from hunting. *(This said "all spawner lists empty" and "creature-free", which the biome JSON has not agreed with for some time: `creature` carries `minecraft:cat` and `minecraft:wolf` at weight 2 each, and `ambient` carries `recompile:pigeon` at weight 3. The design rationale survives the correction - none of the three yields meat, and the mod ships no entity loot table for the pigeon at all - but the flat claim does not, and it is the kind of sentence a reader would take as licence to assume no `spawners` block needs looking at.)*
...
**Worldgen writes it, so only new worlds regrow.** A save made before this shipped has no memory block and its piles stay finite; accepted (owner). That now applies twice more: a world generated before P1.6-R has no Rubble Ground at all and its Stained Ground carries height 0, so an existing yard and dump stay as finite as they were.
...
**Components come in two kinds, and the distinction is the design** (owner, 2026-08-06, locked as P2.11's sibling). **Placeable** components are blocks you stack into a multiblock - Machine Frame, Water Tank, Pump, Copper Pipe, Solar Panel, **Motor**. **Crafting** components are ingredients you spend - the **Bulb** was the first and the **Battery** is the second (#336). The Battery is the odd one: it is **manufactured, never found**, the only component here with no salvage route at all, because what the garbage hands over is a **Depleted Battery** and a dead cell is not a live one. Cutting four dead ones open teaches the blueprint. That is also what settles the Water Tank problem below rather than repeating it - a component named for a capacity that holds nothing reads as broken, and a *depleted* battery holding nothing is simply the noun. They are **inert** (P2.4-R item 6): the Pump moves no fluid and the Motor turns nothing. Their names invite the opposite, which is why it is written down. *(This sentence read "the Solar Panel detects no light" until 2026-09-07, which #72 had made false a month earlier - the panel has a block entity that generates and pushes to its neighbours. It survived because it agreed with a second stale comment on `RCBlocks.SOLAR_PANEL`, and the two together were enough to reason from: a review concluded that a panel on a Hauler Depot generates nothing and merely shades it, and both halves were wrong. Two sources saying the same wrong thing read as corroboration.)* **Two exceptions now. Since 2026-08-18 the Water Tank holds water** (owner ruling from playtest, #229). It is the only component named for a **capacity** rather than an action, and a tank that does not hold reads as broken rather than as a part - two playtesters reported it as a bug ninety minutes apart. **And the Solar Panel has generated since #72**, which is the earlier and larger of the two and was simply never written down here. The carve-out is scoped to that one block and to fluid: a formed machine still redirects a bucket to its core, so the tank is a tank when it is loose and the machine's when it is not. Before this split the vocabulary had exactly one gated part and it did not work - the Pump appeared in **zero** crafting recipes, its only use was one Grass Spreader cell, and both this file and the guidebook claimed it gated the Rain Collector and the Hydroponics Bay, which neither ever did.
...
**eight** recipes, seven of which produce a POWDER (the eighth grinds tires to rubber scrap). Nothing turned powder back into a solid, which is a
...
- **Gold** (#120): E-Scrap -> Circuit Powder (pulverizing, **1:1**) -> gold nugget (Cupola, blasting). *(Said 4:1 until 2026-09-08, which contradicted this file's own rule that `count` is 1 in every pulverizing recipe the mod ships. It is.)*
...
- **Rubber** (#155): tire dumps in the household sprawl. `material_economy.md` has listed rubber as an
  intermediate since P2.2 and it was the only one of the five with no origin at all. A tire is a plain
  slab-shaped block rather than a `SortableBlock` (owner: a tire is not something you pick through), and
  **what it drops is decided by the tool in the loot table** - bare hand gives the tire, a Scrap Knife
  gives the rubber, which is a `minecraft:match_tool` condition and no Java. Fire on a tire is
  netherrack's, through `IBlockExtension.isFireSource` rather than an `infiniburn` tag, so it survives
  rain and does not consume the block. Two things about the FEATURE are worth keeping. **The sprawl
  surface is 86 to 92 percent Mound Ground** (measured: 943 and 884 of 1024 columns in two fresh
  chunks), so the owner's "no Mound Ground under a pile" rule implemented as a survey is not rarity, it
  is a total ban - the dump **retires** the ground instead, converting it to coarse dirt, which is also
  a correctness fix because live Mound Ground would drop Blocks of Garbage onto the tires. And **each
  pile retries its offset** ten times, because piles refuse actual mounds and the first build put all
  of a dump's tires in one blob.
...
- **A viewer must not list an uncraftable multiblock part** (owner, 2026-08-03). A formed cell exists
  ...
  **structurally**: a cell whose formed block differs from the component you place is a transformation,
  ...
  directions.
- **JEI** (`RecompileJeiPlugin`): **seventeen categories**, and the count is deliberately not enumerated here - this line said "three instances" and named three for months while eleven more shipped, which is exactly how a stale list survives: it reads as complete. The list lives in `registerCategories`. What is worth knowing is **when one is needed**, because that is not obvious and has three separate answers.
...
- **Jade** (`RecompileJadePlugin`): **33 provider classes and 42 registrations**, not the three this line listed for months. The three it named are real - a tool hint ("Salvage with a Prybar/Scrap Knife" / "Sort by hand"), sort progress ("Sorted N/max", reading the otherwise-hidden `sorted` blockstate), and the generators (stored FE, rate, burn remaining) - and they are a tenth of it. Missing entirely were every machine's feed and storage providers, `MachineStatusProvider`, `ScrapBinProvider`, `HaulerDepotProvider`, `ScrapHaulerProvider`, `TreeNurseryProvider`, `CompostHeapProvider`, `WorkbenchHintProvider`, `AnimalBaitProvider` and `PaintingNameProvider`. **Read `compat/jade/`, not this sentence** - it is the exact "a list that reads as complete" failure this file warns about elsewhere, and the paragraph about the Pulverizer shipping with zero providers "against the Separator's four" contradicted it in the same document. **A provider that needs server-side state is two classes** - an `IServerDataProvider` plus an `IBlockComponentProvider` - because since MC 1.21.6 one class may not be both. `SortableBlock` exposes public read-only accessors (`sortTool`/`sortedCount`/`sortCrumbleAt`) so the compat package can see its protected internals.
...
- **Teardown has a JEI category and has had one since Phase 3 shipped.** `registerCategories` builds a `SalvageCategory` for `TEARDOWN` sized off `TeardownData.all()`, `registerRecipes` feeds it, and the Recompile Workbench is its catalyst. *(This line read "Teardown JEI is deferred to Phase 3 (only an example recipe exists; its locked-recipe overlay is the Phase 3 risk spike)" long after both halves stopped being true - there are **thirteen** files of `"type": "recompile:teardown"`, not one (twelve if the schema's own `example_iron_door` does not count, which is how the components paragraph above states it) example - and it was the most misleading sentence in this file, because it tells a reader a shipped viewer category does not exist and so invites them to build it a second time.)* EMI is not wired.
...
- **`CraftingMenu.stillValid` hard-codes `Blocks.CRAFTING_TABLE`**, so a custom crafting station that does nothing about it opens and closes on the first server tick, which looks exactly like "right-click does nothing". **Do NOT do it by subclassing `CraftingMenu`** - that was this bullet's advice until 2026-09-08 and it cannot work, because vanilla's constructor hard-locks itself to `MenuType.CRAFTING` (see the GUI framework section above, which has said so all along). `ScrapCraftingStationMenu` reimplements crafting over `AbstractContainerMenu` with its own `MenuType` and overrides `stillValid` there. The bullet also named a class called `ScrapCraftingMenu`, which has never existed. Expect the hard-coded-block pattern in other vanilla menus.
...
pins the ordering for **both remaining** mods, ae2 and enderio - it was unasserted for the other two as well, described in a
...
bottle - so the single recipe id `enderio:tank_empty/glass_bottle` joins `EXEMPT_RECIPES` in
`FoundNotCraftedTests`. **By id, and that is the point of it, so do not "tidy" it into the serializer
list.** *(This paragraph said it joins `RETURNS_ITS_OWN_INPUT`, which is a set of SERIALIZERS -
`minecraft:crafting_dye` and `minecraft:smithing_trim` - and naming the wrong list here describes the
exact shape review of #281 rejected: exempting `enderio:tank` wholesale would also cover 19
`tank_fill/*_concrete` recipes and `tank_fill/nutritious_stick`, which are genuine manufacture with a
different output item. None produces a found-only item today, so nothing was broken, but the
exemption would have been wider than the ruling behind it and would silently cover the next one that
did.)* The caveat is
...
**What a pack can change without a mod release is written down**: `docs/pack_extension.md` - the three sewer loot tables and every pull stream, the nine public recipe types, the tag surface, and how to add guidebook entries from another namespace. It also states what a pack CANNOT reach from data, which is the half people discover the hard way: the viewers read the mod's bundled JSON rather than the live registry, so a retuned pull stream is right in-world and stale in JEI.
