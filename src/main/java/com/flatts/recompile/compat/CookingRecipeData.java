...
/**
 * The shared reader for this mod's <b>cooking-shaped</b> recipe types, for the viewers.
 *
 * <p>Extracted when {@code recompile:sintering} arrived (#248) and became the second type borrowing
 * vanilla's cooking schema. {@code VitrifyingData} had carried this whole body privately, and copying
 * it would have doubled a hundred and twenty lines of ingredient parsing whose hard-won corrections -
 * the JSON array branch, the {@code id}-versus-{@code item} spelling, AIR-not-null - would then have
 * had to be maintained twice.
 *
 ...
 *
 * <p><b>An input carries no count, and that is the schema's shape rather than an omission.</b> A
 * vanilla cooking ingredient has no count - one item goes in per cook. Separating and pulverizing draw
 * a feed count because {@code count} is their dial; drawing one here would invent a number the recipe
 * cannot express.
 */
public final class CookingRecipeData {

    /**
     * One row: what the machine accepts, what it becomes, and how long it takes.
     *
     * <p>{@code inputs} is a LIST because an ingredient may be a tag, and JEI cycles a multi-item input
     * natively - so a tag-input recipe shows what it really accepts rather than one arbitrary member.
     */
    ...
    private CookingRecipeData() {
    }

    /** Every bundled recipe of one cooking-shaped type. */
    public static List<Entry> read(String type, int defaultTicks) {
        List<Entry> out = new ArrayList<>();
        for (JsonObject recipe : RecipeFiles.ofType(type)) {
            ...
            // Chance 1: a cook is not a roll. The same thing comes out every time, which is why an
            // odds column beside every row would be noise.
            out.add(new Entry(inputs, List.of(new SortingData.Weighted(result, 1.0F)),
                recipe.has("cookingtime") ? recipe.get("cookingtime").getAsInt() : defaultTicks));
        }
