...
/**
 * A small patch of {@code garbage_mycelium} with dump mushrooms on it (design P1.9):
 * the forageable food substrate that dots the coarse-dirt plain between mounds. The
 * mycelium replaces the surface dirt block; a few dump mushrooms sprout on top. The
 * player can't pick up the mycelium (it drops nothing) - only the mushrooms.
 */
public class MyceliumPatchFeature extends Feature<NoneFeatureConfiguration> {

    private static final int MIN_RADIUS = 1;
    private static final int MAX_RADIUS = 3;
    private static final float MUSHROOM_CHANCE = 0.4F;
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        BlockState mycelium = RCBlocks.GARBAGE_MYCELIUM.get().defaultBlockState();
