 ...
 * {@code garbage_mycelium} (and coarse dirt) in any light - its own {@link #mayPlaceOn}
 * skips the vanilla mushroom light check. Worldgen places it; the player forages it by
 * breaking it, which drops the edible {@code dump_mushroom} item. The block itself has
 * no block-item (not placeable from the inventory - farming is a later, knowledge-gated tier).
 */
public class DumpMushroomBlock extends VegetationBlock {
    ...
    protected boolean mayPlaceOn(BlockState state, BlockGetter level, BlockPos pos) {
        return state.is(RCBlocks.GARBAGE_MYCELIUM.get()) || state.is(BlockTags.DIRT);
    }
}
