...
/**
 * Where a sewer's pieces land and what they carve (#90).
 *
 ...
 *
 * <p>Split out of {@code SewerTests} at #223. Its siblings are {@link SewerLifeTests} (who lives down
 * there and where they may spawn) and {@link SewerLootTests} (what the structure pays out). The seam is
 * real rather than alphabetical: nothing here reads a loot table and nothing here counts a mob.
 */
final class SewerShapeTests {

    private SewerShapeTests() {
    }
    ...
    static void register() {
        ...
        RCGameTests.test("a_sewer_branches_and_descends", 20, helper -> {
            ...
            for (long seed = 0; seed < SewerFixtures.SEEDS; seed++) {
                List<StructurePiece> pieces = SewerFixtures.layout(seed);
                if (pieces.size() > 3) {
                    branched++;
                }
            ...
            helper.assertTrue(branched > SewerFixtures.SEEDS / 2,
                "only " + branched + " of " + SewerFixtures.SEEDS + " sewers grew past three pieces - the graph is "
                    + "not branching, so this is a corridor rather than the sprawl the structure is for");
            helper.assertTrue(multiLevel > SewerFixtures.SEEDS / 2,
                "only " + multiLevel + " of " + SewerFixtures.SEEDS + " sewers reached more than one level - without "
        ...
        RCGameTests.test("a_sewer_grows_in_all_four_directions", 20, helper -> {
            ...
            helper.assertTrue(north && south && west && east,
                "over " + SewerFixtures.SEEDS + " sewers the branches only reached north=" + north + " south=" + south
        ...
        RCGameTests.test("a_sewer_is_bounded", 20, helper -> {
            ...
            helper.assertTrue(worst > 16,
                "the widest of " + SewerFixtures.SEEDS + " sewers was only " + worst + " blocks across - that is not a "
        ...
        RCGameTests.test("no_barrel_stands_in_the_entrance_shaft", 40, helper -> {
            ...
            String table;
            try (var in = SewerShapeTests.class.getResourceAsStream(
        ...
        RCGameTests.test("sewers_get_their_access_chambers", 20, helper -> {
            ...
            helper.assertTrue(withRoom * 100 / SewerFixtures.SEEDS >= 100,
                "only " + withRoom + " of " + SewerFixtures.SEEDS + " sewers contain an access chamber, and the loot "
            ...
            helper.assertTrue(rooms <= SewerFixtures.SEEDS * 4,
                "averaging " + (rooms / (double) SewerFixtures.SEEDS) + " access chambers per sewer, which is a "
