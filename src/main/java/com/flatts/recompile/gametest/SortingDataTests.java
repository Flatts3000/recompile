...
final class SortingDataTests {
    ...
    private static void registerTeardownDataGuard() {
        ...
        RCGameTests.test("every_bundled_teardown_is_reachable_by_its_input", 20, helper -> {
            ...
            for (var entry : all) {
                // IDENTITY, not non-null. Asking for an item taken out of all() can only ever find
                // something, so a null check here cannot fail and says nothing. What the failure
                // message below actually describes is the lookup returning the WRONG entry, which
                // is what two teardowns sharing an input would cause.
                helper.assertTrue(
                    com.flatts.recompile.compat.TeardownData.forInput(entry.input().getItem())
                        == entry,
                    "forInput handed back a different teardown for " + entry.input()
                        + " than the one that declares it, so one of the two is invisible to every "
                        + "viewer");
            }
            helper.assertTrue(
