...
**Status: SHIPPED in #122 (2026-08-03, for #119); #187 later took the sorting mode off it.** `SeparatorCoreBlock`, `SeparatorChamberBlock` and `SeparatorPartBlock` exist with finished art, including the animated `separator_bay_0..3{,_running}` textures and their `.mcmeta` files, plus five Jade providers. The art half of the gem tier's machine. Mechanics live in
...
| Chamber tiling | **One tile, repeated across all three cells.** Not a continuous strip. *Superseded in the build: the bay is 2x2 and wears one 32px image quartered across its four cells (section 3).* | A shredder chamber genuinely is repetitive, the footprint is not confirmed yet, and a positional mural would have to be re-authored the moment it changes |
...
The mod shipped **no `.mcmeta`** when this was written, so this was a first. It costs nothing and needs no exception.
...
**3 wide x 2 deep x 2 tall**, twelve cells, **four distinct types** (built 2026-08-03, #122; the
blueprint is `SeparatorCoreBlock.createBlueprint`):

| Type | Count | What it is |
|---|---|---|
| Bay | 4 | A 2x2 square of the top, beside a housing column. Recessed well, animated comb, **one image across four blocks** |
| Housing | 6 | Painted plate. Static. The top column beside the bay, the back row (one cell of which takes the Motor since 2026-08-06), and the front cell past the chute |
...
*Superseded by the build (section 3): the bay shipped as a 2x2 square wearing one image quartered
(`separator_bay_0..3`, each with a `_running` strip), and every other face wears the whole-machine
skin (`separator_skin_*`, see `multiblock_system_spec.md`). The reasoning below is the record.*
...
- ~~Footprint confirmation. 3x2x2 is a proposal, not a measurement.~~ Built as 3x2x2 with a 2x2 bay (section 3).
