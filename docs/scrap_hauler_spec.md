...
4. It searches a square of chunks around the Depot, sized from the Depot's screen, and takes **whole blocks**.
...
| 31 | That radius is in **CHUNKS**, a square on the chunk grid, not a circle of blocks |
| 32 | The player **sets the chunk radius in the Depot's GUI**, up to a config maximum |
...
| 33 | Audio is **synthesised, not sourced**, from a declared voice |
| 34 | The machine voice is **heavy**: air-led, no sustained pure tone |
...
**Model and animations are authored in this repo, in code. Sounds are SYNTHESISED.** (owner,
2026-09-05 - this reversed the same day's earlier "sounds we can source", see ruling 33)
...
| **Audio** | synthesised (`sfxgen`) | four sounds: idle loop, pickup, deploy, recall |
...
**Audio now has a rule, and this feature is what forced it** (owner, 2026-09-05). It is the
texture rule's shape with a different answer at the end: **synthesised by `sfxgen`, never sourced or
sampled, only the finished Ogg committed, auditions in gitignored `gen/`, approval explicit.** What
follows is the question as it stood before that was settled.

Textures have a hard rule: generated by texgen, never
...
- **The numbers.** The rule is ruling 29: size one Hauler to feed roughly one Trommel, which eats a
  ...
  a work area of 3x3 chunks. **The dial that matters is rate against regrowth** - faster and it strips the
...
| Rulings 31 and 32, the area | A square of chunks around the Depot, read off the **heightmap**: one lookup per column, the block under `MOTION_BLOCKING`, inside `VERTICAL_REACH` of the Depot. `inWorkArea` is a pure predicate (`>> 4`, which floors, not `/ 16`, which truncates) pinned by `ScrapHaulerAreaTest`. The setting is `HaulerDepotBlockEntity.chunkRadius`, adjusted by two more button ids (`RADIUS_DOWN_BUTTON`, `RADIUS_UP_BUTTON`), clamped on the server to `RCConfig.HAULER_MAX_CHUNK_RADIUS` on every read | 256 cheap lookups per chunk instead of a cube of block reads, and the right shape for what mounds are: piles of `FallingBlock`s whose top is always the exposed one. A pile under a roof is invisible to it, deliberately. Clamping on read means a lowered config ceiling takes effect on existing Depots with no migration; `the_work_area_is_set_from_the_screen_and_clamped_on_the_server` pins the clamp |
