...
Nine public recipe types. A pack writes these the way it writes a vanilla one, in
`data/<pack>/recipe/`:
...
Every one of these is data, so a pack extends the behaviour without a mod release. **The mod ships 41
