...
All twelve custom machine screens are recorded exceptions, the twelfth being the Freight Terminal (#387, no vanilla screen shows a delivery manifest); a thirteenth would need a reversal of its own. (This said eight and a ninth; the market's two terminals shipped the next day, with the reversal recorded in `market_spec.md` section 5, and the Hauler Depot followed as the eleventh, with its reversal recorded in `docs/scrap_hauler_spec.md` section 8.) The
