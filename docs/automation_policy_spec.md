...
| **Slag Furnace** | `AbstractFurnaceBlockEntity` | sided, vanilla | sided, vanilla | **Parity with `minecraft:furnace`.** Three slots, so it subclasses vanilla wholesale; `Capabilities.Item.BLOCK` is registered through a `WorldlyContainerWrapper`. The reason is stated at the registration and is about the CHAIN rather than about consistency: the Cupola rakes slag into a slot and something has to carry it here, so a machine no pipe could reach would break an already-automated chain at its last link. `every_container_block_declares_its_automation` caught the omission on its first run. |
| **Sintering Kiln** | `AbstractFurnaceBlockEntity` | sided, vanilla | sided, vanilla | **Parity with `minecraft:furnace`**, on the Slag Furnace's terms and for the same reason. Blaze powder comes out of a Pulverizer and is pressed into a briquette at a bench; a kiln nothing could feed would break that chain at its last link. |
| **Filing Cabinet** | `RandomizableContainerBlockEntity` | in + out | in + **out**, **non-sided** | Registered as `VanillaContainerWrapper.of(be)`, so the handler ignores side entirely. `canPlaceItem` filters INSERTION to blueprints and Spawn Egg Fragments (the Idea Fragment, renamed in P3.10) - a reference shelf, not a worse Scrap Barrel - and it is on the container so the hopper and pipe paths both obey it. **Nothing filters EXTRACTION**: it is not a `WorldlyContainer`, so there is no `canTakeItemThroughFace` to refuse, and a hopper under a cabinet drains the blueprints out of it. Whether that is right is **open question #355** and this row deliberately does not answer it. What the row does is put the behaviour in the table, which is what this page exists for. |
...
| **Freight Terminal** | `WorldlyContainer` (9-slot landing strip) | **in only** | **in only**, **null side refused** | Automation is the point (#387, owner 2026-09-06): freight is fed by a factory, which is the mechanical difference from the Sell Terminal. `getSlotsForFace` offers all nine slots on every face, `canPlaceItem` accepts only what the current freight phase still wants (so a hopper backs up visibly instead of the terminal eating a stack), and `canTakeItemThroughFace` is false for every slot, so a delivery can never be pulled back out. A **non-sided** query gets no handler, the Tree Nursery's reason: `WorldlyContainerWrapper.extract` skips the face check when the side is null. Deliveries are consumed by the ticker rather than stored. Also a conditional **sink** in `#scrap_connectable` (#393). `the_terminal_refuses_what_the_phase_does_not_want` and `nothing_can_be_pulled_back_out_of_the_terminal` pin the two halves. |
| **Sell Terminal**, **Buy Terminal** | *no block entity* | **none** | **none** | Per-player terminals against a ledger (#311, `docs/market_spec.md`): the balance is a player attachment and the sell grid is menu-local, so there is nothing for a pipe to reach. `a_hopper_against_the_sell_terminal_moves_nothing` and `a_hopper_against_the_buy_terminal_moves_nothing` keep it that way. |
| **Compost Heap**, **Recompile Workbench**, **Scrap Crafting Table** | plain `BlockEntity` | n/a | n/a | Not `Container`s. Nothing to expose. |
| **Separator** | *no container at all* | **none** | **none** | Closed on both doors, and it **joined `#scrap_connectable` anyway** (2026-08-03) without opening either. It is a SOURCE: it pushes separated material into the cluster and can never receive, because routing only ever lands in a Scrap Bin, the Scrap Barrel by block id, or a Freight Terminal, and this machine has no `Container` to land in. Its formed cells are RELAYs, in the tag only so a bin against any face of the machine joins the cluster. **Being reachable and being writable are different questions** - the machine reaches out at both ends (it swallows what lands in its bay, drains a container on it, and pushes into the network or its chute) and is still reachable-into by nothing. |
...
| **Burner Generator** | `WorldlyContainer` (5 fuel slots) | **fuel in, nothing out** | **fuel in, nothing out**, plus **energy out** | Has a bespoke screen for its power meter (see `docs/gui_notes.md` on the custom screens, each a recorded exception); the automation rules below are unaffected by that. `canPlaceItem` is "is this fuel", so neither a player nor a pipe can park something unburnable in the buffer, and `canTakeItemThroughFace` refuses every face - a pipe pulling fuel back out of the generator it just filled is nobody's intent. **Null side returns no handler**, the Burn Barrel's lesson: `WorldlyContainerWrapper` skips `getSlotsForFace` entirely on a non-sided query. |
