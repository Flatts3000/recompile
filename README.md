...
Four systems, four jobs, no overlap. **Teardown** yields function. **Sorting** yields bulk materials. The **market** is the only source of knowledge - every Blueprint is bought with scrip you earned selling goods. **Freight quotas** are progression: deliver the named goods a rung asks for and the next rung opens.
...
**Alpha - released.** **v0.20.0** shipped 2026-09-07 to CurseForge and [GitHub Releases](https://github.com/Flatts3000/recompile/releases), built against MC `26.1.2` / NeoForge `26.1.2`. Recompile is the CurseForge ModJam 2026 ("Echoes of the Past") entry.
...
**The long-open knowledge-vs-function question was decided in favour of FUNCTION** (P3.10, 2026-09-06). Teardown was the mod's knowledge system from 2026-08-02 until then: tear something down, collect Idea Fragments, craft a Blueprint. That is gone. Every Blueprint is bought at the Buy Terminal now, and what teardown yields instead is the working component an object is built around.
...
The data spine - the public `recompile:teardown` recipe type - has been in place since day one, so none of this was retrofitted into a live schema. Its `teaches` field is still parsed and is now unused by every shipped recipe; a pack may still use it.
