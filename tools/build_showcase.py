...
def build_camera_function(scene: Scene) -> str:
    """Just the camera pose, as its own function.

    Exists because the shot depends on where the player happens to be LOOKING, and between placing a
    scene and grabbing the frame there is a pause for chunks to rebuild. Alt-tab into the window and
    nudge the mouse in that window and the screenshot is quietly a different picture. Re-posing
    immediately before the grab closes the gap to nothing, and costs one command.
    """
    ...
    return "\n".join([
        ...
        f"# the camera for {scene.name}, on its own so a shot can re-pose just before grabbing",
        "",
...
def main() -> None:
    ...
    for scene in SCENES:
        ...
        cam_path = OUT / "function" / "showcase" / f"{scene.name}_camera.mcfunction"
        cam_path.write_text(build_camera_function(scene), encoding="utf-8", newline="\n")
